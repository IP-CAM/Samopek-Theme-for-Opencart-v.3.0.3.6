...
class samopek_ModelCatalogProduct extends ModelCatalogProduct {
    ...
    public function getProducts($data = array()) {
        ...
        if (isset($data['filter_sales']) && $data['filter_sales'] == 1) {
            $sql .= " LEFT JOIN " . DB_PREFIX . "product_special p2sp ON p.product_id = p2sp.product_id LEFT JOIN " . DB_PREFIX . "product_discount p2di ON p.product_id = p2di.product_id";
        }
        ...
        if (isset($data['filter_sales']) && $data['filter_sales'] == 1) {
            $sql .= " AND (p.product_id = p2sp.product_id OR p.product_id = p2di.product_id)";
        }

        // MAKE0216 Always get only in stock items
        //if (isset($data['filter_stock']) && $data['filter_stock'] == 1) {
    ...
    // MAKO0216!!! Update query to not return products with quantity = 0
